...
# 24개의 클래스 동작 이름을 정의합니다. 필요에 따라 이름을 변경할 수 있습니다.
actions = [f'{i}' for i in range(1, 22)]
seq_length = 30
secs_for_action = 20  # 각 동작당 데이터 수집 시간을 20초로 설정
        ...
        while time.time() - start_time < secs_for_action:
            ...
            if result.multi_hand_landmarks is not None:
                for res in result.multi_hand_landmarks:
                    ...
                    angle_label = np.array([angle], dtype=np.float32)
                    angle_label = np.append(angle_label, 5)  # 라벨을 5로 고정
